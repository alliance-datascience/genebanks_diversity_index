...
#  dist_prop$GRIN_ID <-    accepted_grin_tax_list$grin_id_cur
  ###############################################################################
  message("OBTAINING NATIVE COUNTRIES (ISO3) FOR SPECIES MATCHING WITH GRIN TAXONOMY")
 ...
 dist_prop <- 
  ...
  #message("CALCULATING SIMPSON FOR COUNTRIES")
...
#"
  ...
  if (length(dist_prop$ncounts_Country[which(dist_prop$status == "wild")]) >
      0) {
    SIMPSON_W <- abdiv::dominance(dist_prop$ncounts_Country[which(dist_prop$status ==
                                                                       "wild")]) #*
  } else {
    SIMPSON_W <- 0
  }
  ...
  if (length(dist_prop$ncounts_Country[which(dist_prop$status == "landrace")]) >
      0) {
    SIMPSON_L <- abdiv::dominance(dist_prop$ncounts_Country[which(dist_prop$status ==
                                                                    "landrace")]) #*
  ...
  if (length(dist_prop$ncounts_Country[which(dist_prop$status == "hybrid")]) >
      0) {
    SIMPSON_H <- abdiv::dominance(dist_prop$ncounts_Country[which(dist_prop$status ==
                                                                    "hybrid")])
  ...
  if (length(x) > 0) {
    ATK_W <- 1 - DescTools::Atkinson(x,parameter = 0.5)
  } else {
    ATK_W <- NA
  }
  rm(x)
  ...
  if (length(x) > 0) {
    ATK_L <- 1 - DescTools::Atkinson(x,parameter = 0.5)
  } else {
    ATK_L <- NA
  }
  rm(x)
  ...
  if (length(x) > 0) {
    ATK_H <- 1 - DescTools::Atkinson(x,parameter = 0.5)
  } else {
    ATK_H <- NA
  }
  rm(x)
  ...
  ##############################################################################
  #NEW SCORE 
  IT3_W  <- NA#(1/SIMPSON_W)*ATK_W #(GINI_SIMPSON_W +(PROPSP1R_W*ATK_W) + SGRIN_W)/3
  IT3_L  <- NA#(1/SIMPSON_L)*ATK_L#(GINI_SIMPSON_L +(PROPSP1R_L*ATK_L) + SGRIN_L)/3
  IT3_H  <- NA#(1/SIMPSON_H)*ATK_H#(GINI_SIMPSON_H +(PROPSP1R_H*ATK_H) + SGRIN_H)/3
  
  IT3_FINAL_MEAN <- (IT3_W + IT3_L) / 2
  ##############################################################################
  TAX_DF <- data.frame(matrix(nrow = 3, ncol = 13))
  colnames(TAX_DF) <- c(
    "type",
    "nTaxa",
    "nRecords",
